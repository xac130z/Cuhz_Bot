...
export async function POST(request) {
  try {
    ...
    if (existing.length > 0) {
      ...
      const msg = alreadyFulfilled
        ? `CuhzBot has already been added to ${row.channel_login}.`
        : `✅ Request received! CuhzBot will be added to ${row.channel_login} in less than 48 hours for free.`;
