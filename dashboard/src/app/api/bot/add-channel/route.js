...
export async function POST(request) {
  try {
    ...
    // If the row exists, behave nicely:
    // - enabled: keep enabled, return info
    // - pending: keep code
    // - disabled: re-enable onboarding by setting pending + new code
    ...
    if (existing.length > 0) {
      ...
      if (row.status === "enabled") {
        return Response.json({
          ...
          verifyCode: null,
          message:
            "Channel already verified. Bot should already be in your chat.",
        });
      }

      if (row.status === "pending" && row.verify_code) {
        // Ensure ownership link is set for the current user
        await sql`
          UPDATE bot_channels
          SET user_id = ${user.id}
          WHERE id = ${row.id}
