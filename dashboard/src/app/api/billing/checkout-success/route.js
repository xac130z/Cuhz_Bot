...
export async function GET(request) {
  try {
    ...
    if (!userId) {
      const dest = `${appUrl}/pricing?upgrade_error=not_authenticated`;
      return Response.redirect(dest, 302);
    }

    if (!sessionId) {
      const dest = `${appUrl}/pricing?upgrade_error=missing_session`;
      return Response.redirect(dest, 302);
    }
    ...
    if (!sessResp.ok) {
      ...
      const dest = `${appUrl}/pricing?upgrade_error=session_lookup_failed`;
      return Response.redirect(dest, 302);
    }
