...
// GET /api/admin/users
// Admin-only listing of users with aggregates
// Query params: limit (1..50), cursor (user.id), q (search username/display_name/twitch_id), role, plan (pro|free), subscription_status
export async function GET(request) {
  try {
    ...
    const role = url.searchParams.get("role"); // user | streamer | admin
    const plan = url.searchParams.get("plan"); // pro | free
    ...
    // Build dynamic query (function form)
    let text = `
      WITH agg AS (
    ...
    // Order + pagination (fetch one extra for nextCursor)
