...
// Query params: limit (1..50), cursor (id), q (username/display_name/twitch_id), method (ai|upload), style, dateFrom (ISO), dateTo (ISO)
// Admin only. Returns recent generations across all users with optional filters.
export async function GET(request) {
  try {
    ...
    const method = url.searchParams.get("method"); // ai | upload
    const style = url.searchParams.get("style"); // text
    const dateFrom = url.searchParams.get("dateFrom"); // ISO or yyyy-mm-dd
    const dateTo = url.searchParams.get("dateTo"); // ISO or yyyy-mm-dd
    ...
    // Build dynamic query using function-form to avoid nested template tags
    let text =
      "SELECT g.id, g.method, g.prompt, g.image_url, g.created_at, g.style, g.user_twitch_id, u.username, u.display_name " +
      "FROM chain_generations g LEFT JOIN users u ON u.twitch_id = g.user_twitch_id WHERE 1=1";
    const values = [];
    ...
    // fetch +1 to determine next page presence
