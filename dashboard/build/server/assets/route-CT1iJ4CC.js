...
import { e as ensureTwitchBotUserColumns } from "./twitch-bot-Bc9aEPa2.js";
import "@neondatabase/serverless";
import "node:crypto";
function parseCommand(text = "") {
  const t = (text || "").trim();
  const lower = t.toLowerCase();
  if (lower === "!cuhz" || lower === "!cuhz help" || lower === "!chain" || lower === "!chain help") {
    return {
      cmd: "help"
    };
  }
...
function isUnsafePrompt(text = "") {
  ...
  return [/ignore\s+(all\s+)?(previous|prior|system|developer)\s+(instructions|messages|prompt)/i, /(reveal|show|print|repeat|leak)\s+(the\s+)?(system|developer|hidden|secret|api|oauth)/i, /jailbreak|prompt\s*injection|system\s*prompt/i, /(?:password|api[_ -]?key|oauth[_ -]?token|seed phrase|private key)/i].some((pattern) => pattern.test(value));
}
  ...
  try {
    ...
    if (isBotAuthed) {
      ...
      if (!owner) {
        return Response.json({
          handled: true,
          reply: "Bot is not enabled for this channel yet. Add it on the dashboard first."
        }, {
          status: 403
        });
      }
      if (!owner.bot_enabled) {
        return Response.json({
          handled: true,
          reply: "Bot is disabled for this channel."
        }, {
          status: 403
        });
      }
    ...
    if (parsed.cmd === "help") {
      return Response.json({
        handled: true,
        reply: "Commands: !cuhz help · !chain <prompt>. Examples: !chain astronaut with CUHZ chain"
      });
    }
    if (parsed.cmd === "generate") {
      ...
      if (res.status === 429) {
        ...
        const todayCount = data2?.todayCount ?? "";
        const dailyLimit = data2?.dailyLimit ?? "10";
        return Response.json({
          handled: true,
      ...
      if (!res.ok) {
        ...
        return Response.json({
          handled: true,
          reply: "Sorry, I couldn't make that right now."
        }, {
          status: 200
        });
      }
      ...
      const reply = imageUrl ? `Here you go → ${imageUrl}` : "Done. Check the gallery soon.";
      return Response.json({
        handled: true,
        reply,
        imageUrl
