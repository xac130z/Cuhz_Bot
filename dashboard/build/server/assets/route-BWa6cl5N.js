...
async function GET(request) {
  ...
  if (!code) {
    ...
    const html = `<!doctype html><html><body><script>window.location.replace(${JSON.stringify(fallback)});<\/script><noscript><a href="${fallback}">Continue</a></noscript></body></html>`;
  ...
  if (!process.env.TWITCH_CLIENT_ID || !process.env.TWITCH_CLIENT_SECRET) {
    ...
    const html = `<!doctype html><html><body><script>window.location.replace(${JSON.stringify(fail)});<\/script><noscript><a href="${fail}">Continue</a></noscript></body></html>`;
    const headers = new Headers({
      "Content-Type": "text/html; charset=utf-8",
      Location: fail
    });
  ...
  try {
    ...
    if (!tokenResponse.ok) {
      console.error("Twitch token exchange failed", tokenResponse.status, tokenResponse.statusText);
    ...
    if (!userResponse.ok) {
      console.error("Twitch user fetch failed", userResponse.status, userResponse.statusText);
      throw new Error(`Failed to get user info from Twitch: [${userResponse.status}] ${userResponse.statusText}`);
    }
    ...
    const html = `<!doctype html><html><body><script>window.location.replace(${JSON.stringify(finishUrl.toString())});<\/script><noscript><a href="${finishUrl.toString()}">Continue</a></noscript></body></html>`;
    ...
    const html = `<!doctype html><html><body><script>window.location.replace(${JSON.stringify(failUrl)});<\/script><noscript><a href="${failUrl}">Continue</a></noscript></body></html>`;
    const headers = new Headers({
      "Content-Type": "text/html; charset=utf-8",
      Location: failUrl
    });
