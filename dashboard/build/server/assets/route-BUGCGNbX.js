...
async function PATCH(request) {
  try {
    ...
    const nextUrl = webhookUrl === "" ? null : webhookUrl || user.discord_webhook_url || null;
    const nextAuto = typeof autoPost === "boolean" ? autoPost : Boolean(user.discord_auto_post);
    await sql`
      UPDATE users
    ...
    return Response.json({
      ok: true,
      settings: {
        discord_webhook_url: nextUrl,
        discord_auto_post: nextAuto
      }
    });
  } catch (err) {
