...
async function POST(request) {
  try {
    ...
    if (generationId) {
      ...
      const twitchIdRows = await sql`SELECT twitch_id FROM users WHERE id = ${user.id} LIMIT 1`;
    ...
    const description = [meta.style ? `Style: ${meta.style}` : null, meta.prompt ? `Prompt: ${meta.prompt}` : null].filter(Boolean).join("\n");
